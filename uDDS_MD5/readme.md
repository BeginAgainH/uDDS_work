...
```C++
	string data;
	getline(cin, data);
	const char * inser_data = data.c_str();
	string encode_data = encode_(data);
	const char * inser_encode = encode_data.c_str();

	instance->data = const_cast<char*>(inser_data);
	instance->encode_data = const_cast<char*>(inser_encode);
```
